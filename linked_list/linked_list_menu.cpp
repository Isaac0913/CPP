...
void linkedListMainMenu()
{
    ...
    do
    {
        ...
        case 3:
        {
            int algo =  lpromptSortAlgorithm(); // ask merge or insertion
        ...
        {
            int algo = lpromptSearchAlgorithm(); // 1=linear, 2=two-pointer
            SearchMode searchMode = (algo == 1 ? MODE_LINEAR : MODE_TWO_POINTER);
            SearchQueryData userQuery = promptSearchQuery(true);

            PerformanceResult result;
            if (choice == 5)
                result = measurePerformance([&]() { searchJobs(searchMode, true, userQuery, jobs); });
            else if (choice == 6)
                result = measurePerformance([&]() { searchJobs(searchMode, false, userQuery, jobs); });
            else if (choice == 7)
                result = measurePerformance([&]() { searchResumes(searchMode, true, userQuery, resumes); });
            else
                result = measurePerformance([&]() { searchResumes(searchMode, false, userQuery, resumes); });
        ...
        case 9: // Job Matching
        {
            int algo = lpromptSearchAlgorithm();
            SearchMode searchMode = (algo == 1 ? MODE_LINEAR : MODE_TWO_POINTER);
